...
#include "../../ImGuiManager/CImGuiHelper.h"
...
{
	m_updateCounter = 0;
	m_updateTime = Timer::GetProgressTime();
	...
	m_updateTime = Timer::GetProgressTime() - m_updateTime;
}
...
{
	m_drawTime = Timer::GetProgressTime();
	m_drawCounter = 0;
	...
	m_drawTime = Timer::GetProgressTime() - m_drawTime;
}
...
void ObjectGenerator::EraseObject()
{
	if (m_listEraseObject.size() > 0)
	{
		...
		{ //消す処理
			...
			EventSystem::EraseObjectFromEvent(objid);
